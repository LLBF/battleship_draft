...
# Starts player 1
def shooting():
    row = int(input("Insert row to shoot: "))
    column = int(input("Inserte column to shoot: "))
...
#machine turn
def machine_shoot():
    row = np.random.randint(0, 11) #Not -1
    column = np.random.randint(0, 11)
    random_position = (row, column )
    if row > 0 or row < 11 or column > 0 or column < 11: # not >1 and <10
...
def run():
    ...
    while turno<4:
        while True:
            print("YOUR TURN")
            print_empty_board_m()
            #print_board_m() #the board with the ships
            shoot = shooting()
            disp_2.append(shoot)

            if np.all(board_m[(shoot[0], shoot[0]), (shoot[1], shoot[1])] == " "):
                empty_board_m[(shoot[0], shoot[0]), (shoot[1], shoot[1])] = "-" #tablero_empty should be board empty of the machine
                ...
                empty_board_m[(shoot[0], shoot[0]), (shoot[1], shoot[1])] = "X" #machine?!
        ...
        while True:
            ...
            if np.all(board[(shoot_m[0], shoot_m[0]), (shoot_m[1], shoot_m[1])] == " "):
                empty_board[(shoot_m[0], shoot_m[0]), (shoot_m[1], shoot_m[1])] = "-"  # tablero_empty should be board empty of the machine
                board[(shoot_m[0], shoot_m[0]), (shoot_m[1], shoot_m[1])] = "-"
                print("WATER")
                break

            elif np.all(board[(shoot_m[0], shoot_m[0]), (shoot_m[1], shoot_m[1])] == "O"):
                empty_board[(shoot_m[0], shoot_m[0]), (shoot_m[1], shoot_m[1])] = "X"  # machine?!
                ...
                if shoot_m in ship1:
                    ...
                    for i in ship2:
                        if ship2[0] == shoot_m or ship2[1] == shoot_m:
                            ...
                            if ship2_m[4] in disp_2 and ship2[5] in disp_2:
                                print("SUNK")
                                break
                    ...
                    for i in ship4:
                        if ship4[0] == shoot_m or ship4_m[1] == shoot_m or ship4_m[2] == shoot_m or ship4_m[3] == shoot_m:
                            if ship4[0] in disp_2 and ship4[1] in disp_2 and ship4[2] in disp_2 and \
                                    ship4[3] in disp_2:
                                print("SUNK")
                                break
